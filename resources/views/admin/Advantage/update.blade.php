...
@section('content')
    ...
    <div class="page-content">
        ...
        <div class="container-xxl">
            <form method="post" action="{{url('admin/advantage/update/'.$advantage['id'])}}" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="col-xl-12 col-lg-12 ">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title"> تعديل ميزات المتجر  </h4>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-lg-6">
...
{{--                                            <input required type="hidden" id="icon" class="form-control" name="icon" value="{{old('icon')}}">--}}
                                            <input required type="hidden" id="icon" class="form-control" name="icon" value="{{old('icon', $advantage['icon'])}}">
                                        </div>
                                    </div>
                                    ...
                                    <script>
                                        ...
                                        const selectedIcon = iconInput.value;  // استرجاع الأيقونة المحفوظة مسبقاً

                                        // تحديد الأيقونة المحفوظة مسبقاً كـ "selected" إذا كانت موجودة
                                        if (selectedIcon) {
                                            icons.forEach(icon => {
                                        ...
                                        // إضافة حدث النقر لتحديد أيقونة جديدة
                                        icons.forEach(icon => {
                                            icon.addEventListener('click', function () {
                                                icons.forEach(i => i.classList.remove('selected'));  // إزالة الحالة selected من جميع الأيقونات
                                                this.classList.add('selected');  // إضافة الحالة للأيقونة المختارة
                                                iconInput.value = this.classList[1];  // تعيين اسم الأيقونة في الحقل المخفي
                                            });
                                        });
                                    </script>
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mb-3">
                                            ...
                                            <textarea name="description" id="" cols="30" rows="10"
                                                      class="form-control">{{$advantage['description']}}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
