    ...
    <div class="wrapper">
                    ...
                    <div class="gap-1 d-flex align-items-center">
                        ...
                        <div class="dropdown topbar-item">
                            <button type="button" class="topbar-button position-relative"
                                ...
                                <i class="ti ti-package text-warning me-2"
                                    style="color: #707693 !important;font-size:24px"></i>
                                @if (isset($stockAlerts) && count($stockAlerts) > 0)
                                    <span
                                        class="position-absolute topbar-badge fs-10 translate-middle badge bg-warning rounded-pill stock-alert-badge">
                                        {{ count($stockAlerts) }}
                                    </span>
                                @endif
                            </button>
                                                    ...
                                                    <div class="flex-shrink-0 me-2">
                                                        @if ($alert['type'] == 'danger')
                                                            <iconify-icon icon="solar:close-circle-bold"
                                                                class="fs-20 text-danger"></iconify-icon>
                                                        @else
                                                            <iconify-icon icon="solar:danger-triangle-bold"
                                                                class="fs-20 text-warning"></iconify-icon>
                                                        @endif
                                                    </div>
                                                    <div class="flex-grow-1">
                                                        <h6
                                                            class="mb-1 fw-semibold {{ $alert['type'] == 'danger' ? 'text-danger' : 'text-warning' }}">
                                                            {{ $alert['title'] }}
                                                        </h6>
                                                        ...
                                                        <small class="text-muted">
                                                            الكمية: <span
                                                                class="fw-bold">{{ $alert['quantity'] }}</span>
                                                            @if ($alert['is_variant'])
                                                                <span
                                                                    class="bg-opacity-10 badge bg-secondary text-secondary me-1">متغير</span>
                                                            @endif
                                                        </small>
                                                    </div>
                                                </div>
                                ...
                                @else
                                    <div class="p-4 text-center">
                                        <iconify-icon icon="solar:check-circle-bold"
                                            class="mb-2 fs-48 text-success"></iconify-icon>
                                        ...
                                        <div data-simplebar style="max-height: 280px;">
                                            ...
                                            @if ($notification['type'] == 'App\Notifications\NewOrder')
                                                <a href="{{ url('admin/order/update/' . $notification['data']['order_id']) }}"
                                                    ...
                                                    <div class="d-flex">
                                                        <div class="flex-grow-1">
                                                            <p class="mb-0 fw-semibold"> رقم الطلب
                                            ...
                                            @else
                                                <a href="{{ url('admin/offer_order/update/' . $notification['data']['order_id']) }}"
                                                    class="py-3 dropdown-item border-bottom">
                                                    <div class="d-flex">
                                                        <div class="flex-grow-1">
                                                            <p class="mb-0 fw-semibold"> رقم الطلب
        ...
        <script>
            function markAllNotificationsAsRead() {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (button && button.closest('.col-auto')) {
                            button.closest('.col-auto').remove();
                        }
                        
                        // Clear notifications list
                        const notificationsContainer = document.querySelector('[data-simplebar]');
            ...
            function showToast(message, type = 'info') {
                // Create toast notification if toastify is available
